...
export const validarNomeCompleto = (nome) => {
  ...
  const partesNome = nomeFormatado
    .split(" ")
...
export const validarTelefoneFixo = (telefone) => {
  ...
  if (telefoneNumeros.length !== 10) {
    return {
      valido: false,
      erro: "Telefone deve ter 10 dígitos (DDD + número)",
    };
  }
...
export const validarCelular = (celular) => {
  ...
  if (celularNumeros.length !== 11) {
    return {
      valido: false,
      erro: "Celular deve ter 11 dígitos (DDD + número)",
    };
  }
...
export const validarFormulario = (formData, isMenorIdade) => {
  ...
  if (isMenorIdade) {
    const validacaoNomePai = validarCampoObrigatorio(
      formData.nomePai,
      "Nome do Pai"
    );
    if (!validacaoNomePai.valido) erros.nomePai = validacaoNomePai.erro;

    const validacaoNomeMae = validarCampoObrigatorio(
      formData.nomeMae,
      "Nome da Mãe"
    );
  ...
  const validacaoEndereco = validarCampoObrigatorio(
    formData.endereco,
    "Endereço"
  );
  ...
  const validacaoConfirmarSenha = validarConfirmarSenha(
    formData.senha,
    formData.confirmarSenha
  );
  if (!validacaoConfirmarSenha.valido)
    erros.confirmarSenha = validacaoConfirmarSenha.erro;
